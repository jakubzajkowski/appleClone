...
const Display=({display,price,device_name})=>{
    const {setDisplayModify} = useContext(iPadPriceContex)
    ...
    if (display?.slice(6,8)=='or'){
        return <div style={{margin:'0 0 15rem 0'}} >
           <h4>Model. Which is best for you?</h4>
            <div>
                <div style={styleBox} onClick={()=>handleSelect(display.slice(8,50),0)} className={(pick?.text==display.slice(8,50)) ? 'border border-primary' : ''}><div><h5 style={{fontSize:'1rem'}}>{device_name}</h5><p style={{fontSize:'0.9rem'}}>{display.slice(8,100)}</p></div><p style={{fontSize:'0.9rem'}}>{`From $${price} or $${Math.floor((price)/12*100)/100}/mo.per month for 24 mo.months before trade‑inFootnote`}</p></div>
                <div style={styleBox} onClick={()=>handleSelect(display.slice(0,5),0.30)} className={(pick?.text==display.slice(0,5)) ? 'border border-primary' : ''}><div><h5 style={{fontSize:'1rem'}}>{device_name}</h5><p style={{fontSize:'0.9rem'}}>{display.slice(0,5)}{display.slice(12,100)}</p></div><p style={{fontSize:'0.9rem'}}>{`From $${price+Math.floor(price*0.30)} or $${Math.floor((price+Math.floor(price*0.30))/12*100)/100}/mo.per month for 24 mo.months before trade‑inFootnote`}</p></div>
            </div>
        </div>
    }
    else{
